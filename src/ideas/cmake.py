...
def extract_symbols(build_dir: Path) -> None:
    # Find *.type files generated by extract_info.cmake
    cmd = ["find", str(build_dir), "-maxdepth", "1", "-name", "*.type"]
    success, output = run_subprocess(cmd)
    if not success:
        raise RuntimeError(f"Finding executables failed: {' '.join(cmd)}\n{output}")
    ...
    for exe in executables:
        ...
        # Remove .type from suffix to get actual executable
        exe = exe[: -len(".type")]
        ...
        if not success:
            raise RuntimeError(
                f"Extracting symbols from {exe} failed: {' '.join(cmd)}\n{output}"
            )
